<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-form class="filter-form">
        <el-form-item class="filter-form-item input-small">
          <el-input v-model="listQuery.nrp" placeholder="Device ID" clearable @change="handleFilter" />
        </el-form-item>
        <el-form-item class="filter-form-item input-small">
        ...
        <el-form-item class="filter-form-item input-small">
          <el-date-picker v-model="listQuery.createdFrom" :picker-options="dateBetween" value-format="yyyy-MM-dd"
            type="date" clearable placeholder="Enrolled Start" @change="handleFilter" />
        </el-form-item>
        <el-form-item class="filter-form-item input-small">
          <el-date-picker v-model="listQuery.createdTo" :picker-options="dateBetween" value-format="yyyy-MM-dd"
            type="date" clearable placeholder="Enrolled End" @change="handleFilter" />
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-dialog title="Filter" :visible.sync="dialogFilter" class="dialog-small">
      <el-form>
        <el-form-item>
          <el-input v-model="listQuery.nrp" placeholder="Device ID" clearable />
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-date-picker v-model="listQuery.createdFrom" :picker-options="dateBetween" value-format="yyyy-MM-dd"
            type="date" clearable placeholder="Enrolled Start" />
        </el-form-item>
        <el-form-item>
          <el-date-picker v-model="listQuery.createdTo" :picker-options="dateBetween" value-format="yyyy-MM-dd"
            type="date" clearable placeholder="Enrolled End" />
        </el-form-item>
      </el-form>
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="dataList" fit @sort-change="sortChange">
      <el-table-column label="Device ID" prop="device_id" sortable="custom">
      ...
      <el-table-column label="Status" prop="status" sortable="custom">
        <template slot-scope="{row}">
          <span>{{ row.adm_usr_active }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Enrolled at" prop="createdAt" sortable="custom" align="center">
        <template slot-scope="{row}">
          <span>{{ row.adm_usr_created_at | dateTimeFilter }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogAddAdmin" :close-on-click-modal="false" class="dialog-small">
      <el-form ref="userListForm" :model="userListForm" :rules="userListRules">
        <el-form-item prop="adm_usr_status">
          <el-select v-model="userListForm.adm_usr_status" placeholder="Status" clearable>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.listLoading = true
      this.dataList = [{
        adm_role_title: "ADMINISTRATOR",
        adm_usr_active: "ACTIVE",
        adm_usr_created_at: "2021-06-16T02:47:21Z",
        adm_usr_created_by: "SYSTEM",
        adm_usr_email: "test@aaa.id",
        adm_usr_first_name: "Admin",
        adm_usr_id: 22,
        adm_usr_last_login: "2022-10-30T12:44:58Z",
        adm_usr_last_name: "Test",
        adm_usr_mobile: "080000000001",
        adm_usr_role_id: 1,
        no: 1,
        role_id: undefined
      }]
    ...
    handleDeleteAdmin(name, username) {
